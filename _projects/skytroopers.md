---
layout: project
title: "The Skytroopers — Heavy-Lift Fixed-Wing UAV"
org: "SAE Aero Design — MIT World Peace University"
status: "Completed"
subtitle: "Led a 30-member multidisciplinary team to MIT-WPU's first international aeromodelling competition, winning Outstanding Technical Presentation and 3rd place Asia-Pacific."
tech: [SolidWorks, CAD/FEA, CFD, ANSYS, XFLR5, Composites, Team Lead]
permalink: /projects/skytroopers/
---

<div class="project-section" markdown="1">

## Overview

The Skytroopers was MIT-WPU's competitive aeromodelling team, entering the SAE Aero Design competition — one of the most prestigious international RC aircraft engineering competitions. I joined as a junior engineer and eventually captained the team, growing it to 30 members across mechanical design, avionics, and operations.

Over three years, the team designed and built multiple aircraft for the Regular and Micro classes, culminating in a competition entry at SAE Aero Design East in Florida (2020) where we won Outstanding Technical Presentation and placed 3rd in Asia-Pacific.

</div>

<div class="project-section" markdown="1">

## Videos

<div class="video-wrapper">
  <iframe src="https://www.youtube.com/embed/6jTqUpqMcG0?start=168" title="SAE Aero Design — Featured Coverage" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</div>
<p class="project-media-caption">Featured in SAE Aero Design coverage — starts at the Skytroopers segment</p>

</div>

<div class="project-section" markdown="1">

## Aircraft Design

<img class="project-featured-img" src="/assets/images/skytroopers/hero.jpg" alt="Final prototype — AeroDesign Florida 2020" style="object-position: center bottom;">

- **Airframe** — Balsa and composite construction with laser-cut ribs and 3D-printed structural joints. Full SolidWorks CAD pipeline from concept to fabrication drawings.
- **Structural Analysis** — FEA in SolidWorks Simulation and ANSYS for wing spar, fuselage bulkhead, and landing gear under max payload and landing loads.
- **Aerodynamics** — XFLR5 for airfoil selection, lift/drag polar analysis, and stability margin verification. CFD in SolidWorks Flow Simulation for fuselage drag estimation.
- **Payload** — Regular class aircraft designed to carry 7.5 kg internal payload with removable bay.

</div>

<div class="project-section" markdown="1">

## Build Process

<img class="project-featured-img" src="/assets/images/skytroopers/frame-grass.jpg" alt="Balsa rib frame structure">

- **Fabrication** — Laser-cut balsa ribs, 3D-printed ABS hubs and control horns, carbon-fibre spar tubes. Doped tissue and MonoKote film covering.
- **Rapid Prototyping** — Alpha prototype built and tested within 6 weeks to validate wing geometry before committing to the final airframe.
- **Ground Testing** — Static load tests on wing assembly to 2× design load, CG verification with ballast, and full control throws sweep before first flight.

</div>

<div class="project-section" markdown="1">

## Micro Class Aircraft

<img class="project-featured-img" src="/assets/images/skytroopers/event-2019-a.jpg" alt="Tandem-wing micro class aircraft on runway">

- **Configuration** — Tandem-wing design for the SAE Micro class, optimised for minimum structural weight while carrying 1.5 kg payload.
- **Design Challenge** — Achieving adequate pitch stability without a conventional tail, using fore/aft wing placement and CG tuning verified in XFLR5.
- **Construction** — Foam-core wings with carbon spar and 3D-printed motor mount. Entire airframe under 400 g empty weight.

</div>

<div class="project-section" markdown="1">

## Competition & Outreach

<img class="project-featured-img" src="/assets/images/skytroopers/florida-tarmac.jpg" alt="Aircraft on Florida competition tarmac">

- **SAE Aero Design East, Florida (2020)** — Won *Outstanding Technical Presentation* award. Placed 3rd at the Asia-Pacific level.
- **Sponsorship** — Raised USD 9,600 by presenting to Cooper Corp. and Poonawalla Group, funding fabrication materials and international travel.
- **Outreach** — Conducted a SolidWorks CAD workshop for 40 students, generating INR 40,000 in revenue for the team.
- **Legacy** — Transitioned to Senior Advisor role, mentoring the next captain and establishing design documentation standards for future teams.

</div>

<div class="gallery-cta">
  <a href="/projects/skytroopers/gallery/" class="btn-gallery">View Full Gallery →</a>
  <span class="gallery-cta-label">28 photos · build process · competition · flight</span>
</div>
